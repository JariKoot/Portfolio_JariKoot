...
- data:     Which file should be used
...
The params can be filled in in the index.Rmd. the first date that can be used is 1-3-2021
...
COVID_filter %>% ggplot() +
  ...
  theme_classic() +
  labs(title = "Recorded COVID-19 cases in the Netherlands",
       x = "")
```
...
COVID_filter %>% ggplot() +
  ...
  theme_classic() +
  labs(title = "Recorded COVID-19 deaths in the Netherlands",
       x = "")
```